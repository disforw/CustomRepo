/*
 * SmartSense Laundry Sensor
 * -------------------------
 * This Device Type is a Samsung Multi-Sensor that has been modified to be a Washing/Dryer Machine Sensor.
 * Mount the device to the side of a Washing/Dryer Machine and use with the "Laundry Monitor App.
 *
 * Version History
 * ---------------
 * 2017-06-08 Initial Re-Code - Changed Vibration to primary
 * 2017-06-09 Hide temperature change from history
 *
 *
 */
...
metadata {
	definition(name: "disforw - SmartSense Laundry Sensor", namespace: "", author: "disforw") {
	...
	tiles(scale: 2) {
		...
		standardTile("acceleration", "device.acceleration", width: 2, height: 2) {
			state("active", label: 'Running', icon: "st.Appliances.appliances1", backgroundColor: "#00a0dc")
			state("inactive", label: 'Idle', icon: "st.Appliances.appliances1", backgroundColor: "#cccccc")
		}
		...
		main(["acceleration", "status", "temperature"])
...
def parse(String description) {
	...
	if (!maps[0]) {
        ...
        map.displayed = false //HIDE THE TEMP CHANGES FROM HISTORY
	}
